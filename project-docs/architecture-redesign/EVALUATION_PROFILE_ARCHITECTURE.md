...
1. **PROFILE (Annual Snapshot):** Resolved strictly at Annual MBO Record initialization. Governs evaluation weights (70/30 or 50/50), competency sets, rating scales, and scoring schemes. **FROZEN FOR THE ENTIRE FISCAL YEAR**.
...
graph TD
    ...
    NEXT_FY["FY2028 Initialization"] --> NEXT_PROF["Resolve New Profile: ASSISTANT_MANAGER (50/50)"]
```
...
* **Mid-Year Promotion:** If an employee is promoted from Chief (70/30) to Assistant Manager (50/50) mid-year, the current fiscal year **strictly retains the initial 70/30 Profile Snapshot**. The new 50/50 Management Profile takes effect in the subsequent Fiscal Year (`FY2028`).
* **Superseded Design Notice:** The previously explored *Controlled Stage Profile Refresh* is **SUPERSEDED**. Profile changes mid-year are strictly prohibited under standard operations.

---

## 3. Evaluation Profile Master Catalog
