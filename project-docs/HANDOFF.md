...
- **Generic Routing Architecture (`DEC-019`, `DEC-020`)**: Twin-Status Engine (`Step N - ALL` / `Step N - ANY`). 6 Generic Slots + Dedicated HR Final Check (45 Native Statuses). Supports In-Flight Approver Reassignment on current record with full audit logging.
