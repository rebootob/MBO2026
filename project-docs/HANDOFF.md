...
- **Controlled Reopen & Revision (`CONTROLLED_REOPEN_REVISION_MODEL = FROZEN`, `SAME_RECORD_NEW_REVISION = FROZEN`, `DEC-022`)**: 1 Employee + 1 FY = 1 Record (`FY2027-0149`). Reopen increments stage revision counter. Superseded revisions archived immutably (Option C Hybrid Model). Full authority matrix (Current Approver returns, Previous Approver requests, HR executes). Approvals invalidated for current revision, preserved in historical audit.
- **Evaluation Profile & Scoring (`EVALUATION_PROFILE_SCORING_ARCHITECTURE = FROZEN`, `DEC-023`, `DEC-024`)**: Annual Profile Freeze Policy (Single profile per FY). Mid-year promotion applies next FY. 4 Profile Families, `WEIGHTED_PART_A_B` algorithm, COCE excluded from score, Half-Up 2-decimal rounding.
