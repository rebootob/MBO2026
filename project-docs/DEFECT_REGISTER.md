...
| **MBO-P02-DEF-011** | P2 | Severity 2 | Rollback exact-record authorization contract was underspecified for multi-record, wrong operation, or mismatched ID cases. | Missing strict operation (`RECORD_DELETE`), target ID formatting, and array length assertions. | `src/core/sandbox-write-guard.js` | Hardened `assertRollbackAuthorization` requiring operation `RECORD_DELETE`, exact single record ID match, and rejecting non-array/multi-record/mismatched requests. | `REC-018` (11 assertions) passing | **`CLOSED`** |
| **MBO-P02-DEF-012** | P2 | Severity 2 | Layer-2 unique conflict handling was tested directly against mock API rather than through service error translator. | Missing service-level error translator for Kintone create errors. | `src/services/annual-record-service.js` | Implemented `AnnualRecordService.translateCreateError` translating Kintone `CB_VA01` into structured `DUPLICATE_MBO_EXISTS`. | `REC-004` passing | **`CLOSED`** |
