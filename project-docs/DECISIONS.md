...
## DEC-041 — App 794 Full Test Sandbox Environment Governance
- **Date**: 2026-08-24
- **Status**: FROZEN (User-Confirmed Environment Governance Decision)
- **Decision**:
  1. **Non-Production Full Test Sandbox:** App 794 (`MBO V2 Sandbox App`) is designated as `APP_794_ENVIRONMENT = SANDBOX` and `APP_794_PRODUCTION = FALSE`. Its explicit purpose is `APP_794_PURPOSE = MBO_V2_DEVELOPMENT_AND_FULL_TESTING`. It is NOT a production application and MUST NOT be treated as permanently read-only.
  2. **Controlled Work-Package Authorized Writes:** App 794 controlled write operations (POST, PUT, DELETE, record creation/update/cleanup, schema/field modifications, Process Management, JS/CSS deployment, REST API integration, workflow, security, migration dry-run, and rollback testing) are permitted ONLY when explicitly planned and authorized by an approved Work Package.
  3. **Default-Deny Model Preserved:** The default safety harness remains `WRITE_ALLOWED_APPS = []`. A Work Package requiring App 794 writes must temporarily authorize `WRITE_ALLOWED_APPS = [794]` for the duration of the authorized operation and define allowed operations, pre-change state, expected changes, test boundaries, read-back verification, and rollback. After completion, the allow-list reverts to `WRITE_ALLOWED_APPS = []`.
  4. **Protected Apps Permanently Read-Only:** Applications 53, 283, 305, 307, 310, 640, 643, 715, and 716 remain protected legacy/reference source systems and are permanently READ ONLY (`POST`, `PUT`, `DELETE`, `DEPLOY` prohibited).
  5. **App 795 Routing Sandbox Governance:** App 795 (`Routing Master Sandbox`) is NOT automatically authorized by App 794 write permission. App 795 writes require separate explicit authorization from a Work Package owning routing work (`WRITE_ALLOWED_APPS = [795]` or `[794, 795]`). Default remains `WRITE_ALLOWED_APPS = []`.
