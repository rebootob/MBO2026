...
> **Last Updated:** 2026-08-24T18:48:00+07:00  
...
| **Phase 3 Migration Contract Commit**| `43f33bd` | Implementation Commit: `docs: complete legacy migration rollback safety contract` |
...
| **Pre-Migration Target Checkpoint** | Capture deterministic pre-batch target checkpoint before batch writes |
| **Exact Migration Manifest** | Exact `Migration_Batch_ID` target record manifest (`Migration_Batch_ID`, `Legacy_Source_App_ID`, `Legacy_Source_Record_ID`, `Target_App_ID`, `Target_Record_ID`, `Migration_Status`) |
| **Post-Write Read-Back Verification** | Kintone REST API post-write read-back verification mandatory; failure yields `MIGRATION_VERIFICATION_FAILED` (Fail Closed) |
| **Batch Success Contract** | `MIGRATION_BATCH_SUCCESS` requires write, read-back, duplicate check, manifest reconciliation, and source/target reconciliation pass |
| **Batch-Only Rollback Contract** | Rollback operates ONLY from `Migration_Batch_ID` target manifest, removing/reverting ONLY target records created by that failed batch. Legacy apps 283..716 remain **PERMANENTLY READ ONLY** |
