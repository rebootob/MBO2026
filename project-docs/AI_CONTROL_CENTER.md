...
> Updated: 2026-09-01 — D1 CLOSED / D2 IN PROGRESS / R3-R20 REVIEWED-NOT-PASS / D2 PRIORITY
...
| D2 Excel + PDF Original/Legacy Format | 🟠 IN PROGRESS | R3-R20 reviewed; bounded R3-R21 corrective proposed |
...
Accepted from R3-R20:
- actual `<dimension>` fingerprinting only in `getWorkbookFingerprint()`; row/cell synthesis removed;
- R3-R19 exact per-sheet print-area binding remains;
- R3-R18 `Sheet1.colsHash` negative proof restored;
- actual observed dimension-tag removal negative proof added;
- earlier privacy/header/typed metadata proof remains present.

Remaining bounded defects:
- `getNoOpParityBuffers()` repairs xlsx-populate output by copying source `<dimension>` tags back when missing, so no-op evidence is not raw observed roundtrip truth;
- `validateWorkbookParity()` now rethrows raw errors instead of deterministically normalizing non-template-source failures to `BLOCKER_WORKBOOK_PARITY_UNRESOLVED`.
...
Intent: preserve accepted R3-R19/R3-R20 work, stop repairing no-op output with source structural evidence, expose raw roundtrip truth, and restore deterministic parity blocker normalization. If raw xlsx-populate output genuinely drops a material dimension tag, report the blocker rather than hiding it. No next blocker auto-start.
