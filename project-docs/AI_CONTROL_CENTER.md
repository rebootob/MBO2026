...
```text
MANIFEST_INTEGRITY = EXPLICIT CANONICALIZATION + REPRODUCIBLE SHA-256
...
DOCUMENT_INDEX_ROUTING = UPDATED FOR PRE1/R1/MANIFEST
```

Machine `executionBlockedUntil` includes all four tooling/policy prerequisites plus scorer approval and fresh pre-write backup/drift verification.
...
Next action: independent Control Plane review of the completed `D3-SBX-MIGRATION-01-PRE1-R1`. Do not auto-start executor implementation or `D3-SBX-MIGRATION-01`.
