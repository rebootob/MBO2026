...
- `project-docs/evidence/D3_SBX_UAT_04_R1_EVIDENCE.md` — Real browser UI read-only vs REST baseline verification evidence record under D3-SBX-UAT-04-R1 (physical desktop WinSta0\Default attach, Chrome PID 30516 authenticated session on App 794 Record 15, 6 local preflight checks verified, 2 explicit REST GET attempts, 1 client in-memory capture, bit-for-bit identical 350/350 fields, $revision = 1, zero drift, zero mutations, zero Kintone writes; PASS / BASELINE VERIFIED / REVIEW REQUIRED; minimal sanitized JSON excerpts and opaque sanitized screenshots).
