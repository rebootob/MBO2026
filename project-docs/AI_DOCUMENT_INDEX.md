...
For D1 status/reopen questions, read `CONFIRMED_BASELINE/D1_CLOSURE.md` first. D1 is frozen PASS/CLOSED unless proven regression or explicit architecture change.

For employee lifecycle work, read `CONFIRMED_BASELINE/EMPLOYEE_LIFECYCLE_CHANGE_POLICY.md` first.

## 4. D2 routing — CURRENT PRIORITY
...
3. `AI_ACTIVE_TASK.md`;
4. `EXCEL_EXPORT.md`;
5. exact current feasibility source/tests named by the active/proposed WP;
6. exact SHA-verified owner templates only when required;
...
Do not broad-read historical evidence, old architecture trees or compatibility snapshots by default. Repository history remains available for audit.
...
Do not trust executor self-report as independent PASS evidence.
