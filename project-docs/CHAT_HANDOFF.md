...
| D2 | 🟠 IN PROGRESS | Preservation PASS/CLOSED; reference-image source PASS/FROZEN; R3-R35 TEST-ONLY proposed |
...
Accepted R3-R34 progress: leading-digit/hyphen/dot prefix negatives now reject; Relationship start-tag scanning consumes malformed/unquoted/stray syntax fail-closed; mixed single/double quoted valid attributes still parse.

Remaining TEST-ONLY blockers:
- `isValidNCName()` is still only an approximation of XML NCName and rejects valid NameChar forms such as middle-dot/combining characters while not representing full XML NameStartChar/NameChar ranges;
- Relationship attribute-name tokenization accepts XML-invalid names such as leading-digit names or malformed multi-colon QName forms;
- the R3-R34 change removed several accepted R3-R33 adversarial assertions instead of retaining them, including wrong-case local-name, nested-child substitution, duplicate required-attribute, namespaced substitute and TargetMode tuple-inequality proof.
...
R3-R35 is proposed only. Do not auto-start it.
