...
> **Audit Date:** 2026-08-24T15:19:00+07:00  
...
1. **Active Dependency Lineage for App 283 & App 310:**
   - **App 283 (Staff & Chief - Rev 1003):**
     - Terminal formula: `total_all = ((total_a + total_b) * 100) / 5`.
     - Active Part A chain: `total_score` $\to$ `total_a = ROUND((total_score * 70) / 100, 2)` $\to$ `total_all`.
     - Field `total_a_0` (`ROUND(...)`) has **0 downstream references** $\implies$ **`DUPLICATE_CALC / LEGACY_UNUSED`**.
     - Active Part B chain: `ratings 1..5 (App 1 & 2)` $\to$ `devide_10 = (ratings) / 10` $\to$ `sum_part_b = ROUND(devide_10 * 0.3, 2)` $\to$ `total_b` $\to$ `total_all`.
     - Field `sum_rating` has 0 downstream references $\implies$ **`DUPLICATE_CALC / SUPPORTING_DISPLAY`**.
     - Status: **`APP283_SCORING_EVIDENCE = VERIFIED`** (Revision 1003).
   - **App 310 (Assistant Manager - Rev 555):**
     - Terminal formula: `total_all = ((total_a + total_b) * 100) / 5`.
     - Active Part A chain: `total_score` $\to$ `total_a = ROUND((total_score * 60) / 100, 2)` $\to$ `total_all` (**60% Part A with standard ROUND 2 decimals**).
     - Field `total_a_0` (`ROUNDUP(...)`) has **0 downstream references** $\implies$ **`DUPLICATE_CALC / LEGACY_UNUSED`**.
...
3. **COCE / Compliance Exclusion Verified 100% in Live Active Formulas:**
   - Across all 8 legacy apps (283, 305, 307, 310, 640, 643, 715, 716), Competency 6 (`rating_1_6` / `rating_2_6`) exists on the form and is evaluated, but is **strictly omitted from `sum_rating` / `devide_10`**.
...
4. **Appraiser Cardinality & Deployed Scaling Model:**
   ...
   - **Executive Apps (640, 715):** **`CURRENT_DEPLOYED_BASELINE = 1 Appraiser`** evaluating 7 Scored Competencies $\implies \text{Formula: } (\sum \text{Ratings}) \times 2 / 14$ (Equivalently $\sum / 7$).
...
| 283 | | `total_a` | `ROUND((total_score*70)/100, 2)` | `total_all` | **`ACTIVE_SCORING_CHAIN`** |
| 283 | | `total_a_0` | `ROUND((total_score*70)/100, 2)` | `(Unreferenced in 283)` | **`DUPLICATE_CALC / LEGACY_UNUSED`** |
...
| 283 | | `sum_rating` | `rating_1_1..5+rating_2_1..5` | `(Unreferenced in 283)` | **`DUPLICATE_CALC / SUPPORTING_DISPLAY`** |
...
| 716 | | `total_a_0` | `ROUND((total_score*70)/100, 2)` | `(Unreferenced in 716)` | **`DUPLICATE_CALC / LEGACY_UNUSED`** |
...
| 310 | | `total_a_0` | `ROUNDUP((total_score*60)/100, 2)` | `(Unreferenced in 310)` | **`DUPLICATE_CALC / LEGACY_UNUSED`** |
...
| **App 310 Part A Rounding** | Suspected `ROUNDUP` from unreferenced `total_a_0` | **`ROUND(..., 2)`** via active downstream field `total_a` | **`KINTONE_ACTIVE_LINEAGE_CONFIRMED`** | Use `ROUND(..., 2)`; classify `total_a_0` as `DUPLICATE_CALC / LEGACY_UNUSED`. |
| **GM / VP Appraiser Cardinality** | 1–2 Appraisers generic | **`CURRENT_DEPLOYED_BASELINE = 1 Appraiser`** evaluated, scaled via `(sum*2)/14` | **`KINTONE_OVERRIDES_DOC`** | Formalize 1-appraiser deployed model; decouple future capacity (1..2) from deployed baseline. |
