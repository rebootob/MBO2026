...
> Authorization Token: `D2-WP004-R1-R2-R2-EVIDENCE-20260902-01`  
...
| `HEADER_EMPLOYEE_NAME_TH` | `"Name - Surname"` | `AT6` | `AT7` | `AT7:BC7` | `partA.header.employeeNameTH` | `UNRESOLVED` | Alternate source field; AT7 is exclusively owned by `HEADER_EMPLOYEE_NAME` to prevent duplicate target ownership. |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `OBJECTIVE_i_TITLE` | `"Objectives (Indicate expected result...)"` | `B${r}` | `B${r}:I${r}` | `partA.objectives[i-1].title` | `UNRESOLVED` | Combined B:I objective/target region; standalone title composition rule unproven without workflow authority. |
...
| `OBJECTIVE_i_SELF_ACHIEVEMENT` | `"1st Appraiser Achievement Level [1-5]"` | `AS${r}` | `AS${r}:AU${r}` | `partA.objectives[i-1].selfAchievement` | `UNRESOLVED` | Header label says 1st Appraiser; Self evaluator translation unproven without workflow authority. |
...
| `OBJECTIVE_i_MANAGER_ACHIEVEMENT` | `"1st Appraiser Achievement Level [1-5]"` | `AS${r}` | `AS${r}:AU${r}` | `partA.objectives[i-1].managerAchievement` | `UNRESOLVED` | 1st Appraiser -> Manager role translation unproven without explicit accepted workflow authority. |
| `OBJECTIVE_i_MANAGER_SCORE` | `"1st Appraiser SCORE"` | `AV${r}` | `AV${r}:AW${r}` | `partA.objectives[i-1].managerScore` | `UNRESOLVED` | 1st Appraiser SCORE -> Manager score role translation unproven without workflow authority. |
...
| `OBJECTIVE_i_GM_ACHIEVEMENT` | `"2nd Appraiser Achievement Level [1-5]"` | `AX${r}` | `AX${r}:AZ${r}` | `partA.objectives[i-1].gmAchievement` | `UNRESOLVED` | 2nd Appraiser -> GM role translation unproven without explicit accepted workflow authority. |
| `OBJECTIVE_i_GM_SCORE` | `"2nd Appraiser SCORE"` | `BA${r}` | `BA${r}:BB${r}` | `partA.objectives[i-1].gmScore` | `UNRESOLVED` | 2nd Appraiser SCORE -> GM score role translation unproven without workflow authority. |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `SUMMARY_WEIGHT_SUM` | `"Total Score : 5"` | `F29` / `BC29` | Merged | None | `UNRESOLVED` | Formula = 0 rule; calculated in Excel formula engine, no renderable secured service path. |
...
- **Self Dynamic Rating Authority Area**: Columns `K:Q` (`K7:Q7`, `K8:Q8`, etc.)
- **Chief Dynamic Rating Authority Area**: Columns `R:X` (`R7:X7`, `R8:X8`, etc.) — Full frozen authority span is `R:X`.

| Competency Block | Description Row | Rating Value Row | Visual Self Target (K:Q) | Visual Chief Merge / Target | Frozen Chief Dynamic Authority | Secured Projection Path | Status | Rationale |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Comp 1: Adaptability** | Row 8 | Row 9 | `K9:Q9` | `R9:W9` (Visual merge) | `R9:X9` | Self: `partB.competencyItems[0].selfRating` / Chief: Unproven key | Self: `PROVEN` / Chief: `UNRESOLVED` | Self rating target K9 is proven. Visual Chief merge is R9:W9; complete frozen Chief dynamic authority is R9:X9. Chief key path is unproven for Employee-Self context. |
| **Comp 2: Problem Solving** | Row 12 | Row 13 | `K13:Q13` | `R13:W13` (Visual merge) | `R13:X13` | Self: `partB.competencyItems[1].selfRating` / Chief: Unproven key | Self: `PROVEN` / Chief: `UNRESOLVED` | Self rating target K13 is proven. Complete frozen Chief dynamic authority is R13:X13. |
| **Comp 3: Customer Focus** | Row 16 | Row 17 | `K17:Q17` | `R17:W17` (Visual merge) | `R17:X17` | Self: `partB.competencyItems[2].selfRating` / Chief: Unproven key | Self: `PROVEN` / Chief: `UNRESOLVED` | Self rating target K17 is proven. Complete frozen Chief dynamic authority is R17:X17. |
| **Comp 4: Additional Value Creation** | Row 20 | Row 21 | `K21:Q21` | `R21:W21` (Visual merge) | `R21:X21` | Self: `partB.competencyItems[3].selfRating` / Chief: Unproven key | Self: `PROVEN` / Chief: `UNRESOLVED` | Self rating target K21 is proven. Complete frozen Chief dynamic authority is R21:X21. |
| **Comp 5: Safety Awareness** | Row 24 | Row 25 | `K25:Q25` | `R25:W25` (Visual merge) | `R25:X25` | Self: `partB.competencyItems[4].selfRating` / Chief: Unproven key | Self: `PROVEN` / Chief: `UNRESOLVED` | Self rating target K25 is proven. Complete frozen Chief dynamic authority is R25:X25. |
| **Comp 6: Compliance / COCE** | Row 28 | Row 29 | `K29:Q29` | `R29:W29` (Visual merge) | `R29:X29` | Self: `partB.competencyItems[5].selfRating` / Chief: Unproven key | Self: `PROVEN` / Chief: `UNRESOLVED` | Self rating target K29 is proven. Complete frozen Chief dynamic authority is R29:X29. |
| **Padding Row** | N/A | Row 30 | `B30:X30` | `B30:X30` | `B30:X30` | None | `PROVEN` | Row 30 is protected non-dynamic padding (`isDynamicWriteTarget === false`). |
...
| `OBJECTIVE_i_TITLE` | Part A `B${24+i}` | `partA.objectives[i-1].title` | Combined B:I region | `UNRESOLVED` | `KEEP_UNRESOLVED` |
| `OBJECTIVE_i_DESCRIPTION` | Part A `B${24+i}` | `partA.objectives[i-1].description` | Combined B:I region | `UNRESOLVED` | `KEEP_UNRESOLVED` |
| `OBJECTIVE_i_KPI` | Part A `J${24+i}` | `partA.objectives[i-1].kpi` | Label Action Plan | `UNRESOLVED` | `KEEP_UNRESOLVED` |
| `OBJECTIVE_i_TARGET` | Part A `B${24+i}` | `partA.objectives[i-1].target` | Combined B:I region | `UNRESOLVED` | `KEEP_UNRESOLVED` |
...
| `OBJECTIVE_i_MANAGER_ACHIEVEMENT` | Part A `AS${24+i}` | `partA.objectives[i-1].managerAchievement` | Label 1st Appraiser | `UNRESOLVED` | `KEEP_UNRESOLVED` |
| `OBJECTIVE_i_MANAGER_SCORE` | Part A `AV${24+i}` | `partA.objectives[i-1].managerScore` | Label 1st Appraiser | `UNRESOLVED` | `KEEP_UNRESOLVED` |
| `OBJECTIVE_i_MANAGER_COMMENT` | Part A `B29` | `partA.objectives[i-1].managerComment` | Shared row 29 | `UNRESOLVED` | `KEEP_UNRESOLVED` |
| `OBJECTIVE_i_GM_ACHIEVEMENT` | Part A `AX${24+i}` | `partA.objectives[i-1].gmAchievement` | Label 2nd Appraiser | `UNRESOLVED` | `KEEP_UNRESOLVED` |
| `OBJECTIVE_i_GM_SCORE` | Part A `BA${24+i}` | `partA.objectives[i-1].gmScore` | Label 2nd Appraiser | `UNRESOLVED` | `KEEP_UNRESOLVED` |
...
| `SUMMARY_WEIGHT_SUM` | Part A `F29` | None | Formula = 0 rule | `UNRESOLVED` | `KEEP_UNRESOLVED` |
...
| `COMPETENCY_b_CHIEF_RATING` | Part B `R${row}` | Unproven key path | Frozen R:X authority | `UNRESOLVED` | `KEEP_UNRESOLVED` |
...
## Mechanical Status Counts & Invariants

- **`PROVEN` (`SAFE_TO_MAP`)**: `18`
- **`UNRESOLVED` (`KEEP_UNRESOLVED`)**: `22`
- **`NO_SECURED_PROJECTION_SOURCE` (`DO_NOT_MAP_NO_PROJECTION_SOURCE`)**: `5`
- **`DUPLICATE_EXCLUSIVE_SAFE_TO_MAP_TARGETS`**: `0` (EXACT ZERO)
- **`SAFE_TO_MAP_WITH_NULL_OR_UNKNOWN_SECURED_PATH`**: `0` (EXACT ZERO — Every `SAFE_TO_MAP` item has a valid secured projection path)
...
3. **Align Objective & Summary Roles**:
   - Keep combined `OBJECTIVE_i_TITLE`, `OBJECTIVE_i_DESCRIPTION`, `OBJECTIVE_i_KPI`, `OBJECTIVE_i_TARGET`, `OBJECTIVE_i_PROGRESS_PERCENT`, `OBJECTIVE_i_DIFFICULTY`, and unproven evaluator roles as `UNRESOLVED` in mapping profile.
   - Keep `SUMMARY_WEIGHT_SUM` as `UNRESOLVED` / non-writable (Formula = 0 rule).
