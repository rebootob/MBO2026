...
2. Unequal appraiser weighting (e.g. 60/40 between appraisers) without separate user approval.
3. Phase 3 JavaScript implementation (deferred to execution work packages).
4. Phase 4 Hoshin dual-level gating.
5. Phase 5 Enterprise App 795 seeding.
...
| Evaluation Group | Profile Family | Applicable Competency Set | Displayed Items | Included Items ($N_{\text{included}}$) | Part A Weight (%) | Part B Weight (%) | Current Deployed Appraisers ($K_{\text{expected}}$) | Layer 1 Appraiser Weights | Authoritative Evidence |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **Staff & Chief** | `PROFILE_STAFF_CHIEF` | `COMP_SET_OPERATIONAL_V1` | 6 | **5** | **70%** | **30%** | 2 | **50% / 50%** | App 283 (Rev 1003) / DEC-035 |
| **Japanese Staff** | `PROFILE_JAPANESE_STAFF`| `COMP_SET_OPERATIONAL_V1` | 6 | **5** | **70%** | **30%** | 2 | **50% / 50%** | App 716 (Rev 1031) / DEC-035 |
| **Assistant Manager**| `PROFILE_MANAGEMENT` | `COMP_SET_MANAGEMENT_V1` | 8 | **7** | **60%** | **40%** | 2 | **50% / 50%** | App 310 (Rev 555: 60/40) / DEC-035 |
| **Section Manager** | `PROFILE_MANAGEMENT` | `COMP_SET_MANAGEMENT_V1` | 8 | **7** | **50%** | **50%** | 2 | **50% / 50%** | App 305 (Rev 596) / DEC-023 |
| **Senior Manager** | `PROFILE_MANAGEMENT` | `COMP_SET_MANAGEMENT_V1` | 8 | **7** | **50%** | **50%** | 2 | **50% / 50%** | App 643 (Rev 503) / DEC-023 |
| **Deputy General Mgr**| `PROFILE_MANAGEMENT` | `COMP_SET_MANAGEMENT_V1` | 8 | **7** | **50%** | **50%** | 2 | **50% / 50%** | App 307 (Rev 579: PMS DGM) |
| **General Manager** | `PROFILE_EXECUTIVE` | `COMP_SET_MANAGEMENT_V1` | 8 | **7** | **50%** | **50%** | **1** (Capacity: 1..2) | **100%** | App 640 (Rev 542) / DEC-035 |
| **Vice President** | `PROFILE_EXECUTIVE` | `COMP_SET_MANAGEMENT_V1` | 8 | **7** | **50%** | **50%** | **1** (Capacity: 1..2) | **100%** | App 715 (Rev 543) / DEC-035 |
...
* **Operational (283, 716):** 70/30 Split, 2 Appraisers (50/50), Denominator 10 (`(sum)/10`).
* **Assistant Manager (310):** 60/40 Split, 2 Appraisers (50/50), Denominator 14 (`ROUND(sum/14, 2)`).
* **Section Manager / Senior Manager / DGM (305, 643, 307):** 50/50 Split, 2 Appraisers (50/50), Denominator 14 (`ROUND(sum/14, 2)`).
...
## 9. App 794 Field Manifest & Gap Analysis
...
## 10. Security Impact & Confidentiality Model
...
## 12. Change Manifest (Current vs Proposed Future)
...
* `test_coce_excluded_dynamic_denominator`: Verify COCE evaluated but omitted, yielding $N=5$ (Ops) or $N=7$ (Mgmt).
* `test_objective_difficulty_achievement_20_cases`: Verify all 20 combinations of Difficulty $\times$ Achievement match live Kintone matrix.
* `test_score_normalization_100_point`: Verify $((A+B)*100)/5$ scale conversion.
* `test_fail_closed_unknown_position`: Verify invalid/empty position halts initialization with `PROFILE_SOURCE_INVALID`.
* `test_fail_closed_ambiguous_position`: Verify non-standard position halts initialization with `PROFILE_MAPPING_AMBIGUOUS`.
* `test_missing_config_fail_closed`: Verify missing/inactive profile config halts initialization.
* `test_confidential_field_security_regression`: Verify employee/requester role cannot access appraiser ratings/comments.
...
## 16. Profile Configuration Storage Decision (Open Decision)
...
## 17. Dependencies & Next Work Package Boundary
