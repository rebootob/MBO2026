...
export function buildStageLogicalSnapshot(record, targetStage, currentStatus) {
  ...
  // Workflow appraisers: strictly normalized from exact persisted route fields
  ...
  if (Array.isArray(rawAppraisers) && rawAppraisers.length > 0) {
    ...
    for (const slotId of patternDef.sourceSlots) {
      ...
      const fieldVal = getVal(slotDef.approverField) ?? (slotDef.legacyField ? getVal(slotDef.legacyField) : undefined);
      let code = '';
  ...
  // Scorers: must come strictly from persisted Effective_Scorer_Slots_Snapshot or Scorers table
  let scorers = [];
