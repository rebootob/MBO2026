  ...
  static projectCombinedExport({ mboRecord, profileCode, competencyItems = [], exportContext }) {
    ...
    const projectedCompetencyItems = (Array.isArray(competencyItems) ? competencyItems : []).map((item, index) => {
      ...
      if (!item || typeof item !== 'object') {
        throw new Error('EXPORT_COMPETENCY_PRESENTATION_UNRESOLVED: Expanded competency item must be an object.');
      }
      ...
      if (ordinal === 7) {
        if (item.code !== 'COMP_LEAD') {
          throw new Error(`EXPORT_COMPETENCY_PRESENTATION_UNRESOLVED: Competency 7 code must be exact COMP_LEAD, found '${item.code}'.`);
        }
        ...
        if (item.code !== 'COMP_STRAT') {
          throw new Error(`EXPORT_COMPETENCY_PRESENTATION_UNRESOLVED: Competency 8 code must be exact COMP_STRAT, found '${item.code}'.`);
        }
        if (typeof item.description !== 'string' || item.description.trim() === '') {
